...
class PromptGeneratorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Prompt Generator v22")
        self.root.geometry("1480x980")
        self.root.minsize(1240, 860)

        self.base_dir = Path(__file__).resolve().parent
        self.config_path = self.base_dir / "prompt_config_v21.json"
    ...
    def _build_ui(self):
        ...
        # Row 2
        ttk.Label(top, text="電影感").grid(row=1, column=0, pady=(10, 0), sticky="w")
        ttk.Combobox(top, textvariable=self.cinema_mode, values=self.cfg("cinematic_presets_order"), width=15, state="readonly").grid(
            row=1, column=1, columnspan=2, pady=(10, 0), sticky="w"
        )
        ...
        tab_wrap.pack(fill="both", expand=False)
    ...
    def _build_nsfw_tab(self):
        self.nsfw_header = ttk.Frame(self.nsfw_tab)
        self.nsfw_header.pack(fill="x", pady=(0, 6))
        ttk.Button(self.nsfw_header, text="全隨機", command=self.reset_nsfw, takefocus=False).pack(side="right")
        ttk.Button(self.nsfw_header, text="全解鎖", command=self.unlock_nsfw, takefocus=False).pack(side="right", padx=5)
        ttk.Button(self.nsfw_header, text="全鎖定", command=self.lock_nsfw, takefocus=False).pack(side="right", padx=5)

        self.nsfw_content = ttk.Frame(self.nsfw_tab)
        self.nsfw_content.pack(fill="x")

        cols = [ttk.Frame(self.nsfw_content) for _ in range(2)]
        for c in cols:
            c.pack(side="left", fill="both", expand=True, padx=6)

        ns = self.cfg("nsfw_selectors")
        groups = [
            ["nsfw_camera", "focus_area", "nsfw_pose_action", "hand_nsfw"],
            ["areola_detail", "breast_pressure", "breast_size", "wetness"]
        ]
        for col_idx, group in enumerate(groups):
            for row, key in enumerate(group):
    ...
    def update_nsfw_tab_state(self, *args):
        ...
        nsfw_index = self.notebook.index(self.nsfw_tab)
        if mode == "off":
            self.notebook.tab(nsfw_index, state="disabled")
            if self.notebook.index("current") == nsfw_index:
                self.notebook.select(self.general_tab)
        else:
            self.notebook.tab(nsfw_index, state="normal")
    ...
    def reset_nsfw(self):
        for v in self.nsfw_dropdown_vars.values():
            v.set("隨機")
    ...
    def choose_tag(self, key, is_nsfw=False):
        ...
        if lock_var.get():
            current = var.get()
            if current == "隨機":
                return self.pick(cfg["隨機"])
            return self.pick(cfg[current])

        picked = self.pick(cfg["隨機"])
        matched = False
    ...
    def build_positive(self):
        p = []
        if self.constant_quality_var.get():
            p += self.cfg("quality")
        p += self.cfg("subject")
    ...
    def build_negative(self):
        if self.lock_neg_var.get():
            return self.neg_text.get("1.0", "end").strip()
        n = (
        ...
        return ", ".join(dict.fromkeys(n))

    def generate(self):
        self.prompt_text.delete("1.0", "end")
        self.prompt_text.insert("1.0", self.build_positive())
        self.neg_text.delete("1.0", "end")
        self.neg_text.insert("1.0", self.build_negative())
